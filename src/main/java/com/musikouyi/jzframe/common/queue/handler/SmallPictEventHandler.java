...
public class SmallPictEventHandler implements WorkHandler<SmallPictEvent> {
    ...
    public void onEvent(SmallPictEvent event) {
        try {
            ...
            int size = SmallPictUtil.generateSmallPict(smallPictEventData.getWidth(), smallPictEventData.getHeight(), ResourceUtils.getURL(Global.uploadFolder).getPath() + File.separator + smallPictEventData.getFilePath(), smallPictEventData.isInnerCut());
            if (size > -1) {
